...
//Description: This is score_board_file.
   ...
   function void compare_transactions();
    ...
    function void report_phase(uvm_phase phase);
       ...
       if(fail_count==0) begin
          ...
          `uvm_error("[SB]"," RESULT:** TEST FAILED **");
          `uvm_info("SB","------------------------------",UVM_NONE);
       end
...
//
//         if(seq_h.pslverr) begin
//            $error("[SB] %0t : Slave error on paddr=0x%0h pwrite=%0b",$time,seq_h.paddr,                        seq_h.pwrite);
//            continue;
//         end
//
//         if(seq_h.pwrite) begin
//            mem[seq_h.paddr]=seq_h.pwdata;
//            $display("[SB] %0t: WRITE mem[0x%0h]= 0x%0h",$time,
//               seq_h.paddr,seq_h.pwdata);
//         end
//         else begin
//            if(!mem.exists(seq_h.paddr)) begin
...
//            end
//
//            if(mem[seq_h.paddr]==seq_h.prdata) begin
//              // pass_count++;
//               $display("[SB] %0t : Pass paddr=0x%0h expected=0x%0h got=0x%0h",
//               $time,seq_h.paddr,mem[seq_h.paddr],seq_h.prdata);
//            end
//            else begin
//              // fail_count++;
